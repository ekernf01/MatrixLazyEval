...
Sometimes it doesn't make sense to store your matrices explicitly. `LazyMatrix` allows you to store them in clever ways that save time and memory. Although using a `LazyMatrix` can slow down selection of subsets -- much like an insert-favorite-procrastination-stereotype-here, everything is done at the last possible moment -- it allows for much faster matrix multiplication. In turn, this facilitates speedups for algorithms that only require matrix-vector products -- for example, the very popular iteratively restarted lanczos bidiagonalization used for PCA in the `irlba` package. 

In the following examples, you start with a large, sparse matrix $M$, and you want to perform simple operations without causing it to become dense. This vignette will compare the performance of `LazyMatrix` against a naive approach using only the `Matrix` package. 
...
You wish to scale the columns of $M$ to a constant $K$.
...
M_scaled_lazy = NewLazyMatrix( components = list("M" = M, "C" = C ), 
                               dim = dim(M),
                               eval_rule  = " C %*% M " )
object.size(M_scaled)
object.size(M_scaled_lazy)
x = rnorm( ncol( M ) )
microbenchmark::microbenchmark({M_scaled      %*% x})
microbenchmark::microbenchmark({M_scaled_lazy %*% x})
```
